...
test('文件名带时间戳，保证每次发布唯一', () => {
  ...
  assert.equal(
    buildApkFileName('dev', 'unified-portal-app', '1.0.0', '1', at),
    'dev-unified-portal-app-1.0.0-1-20260901-1130.apk',
  );
});

test('同一版本两次发布得到不同文件名（immutable 缓存下不会串包）', () => {
  const a = buildApkFileName('dev', 'app', '1.0.0', '1', new Date('2026-09-01T11:30:00+08:00'));
  const b = buildApkFileName('dev', 'app', '1.0.0', '1', new Date('2026-09-01T14:05:00+08:00'));
  assert.notEqual(a, b);
});

test('label 里的空格和斜杠被清理，不会造出坏路径', () => {
  const at = new Date('2026-09-01T11:30:00+08:00');
  const name = buildApkFileName('dev', '统一门户 App/v2', '1.0', '9', at);
...
test('按档清理时不会选中其它档的 APK', () => {
  const files = [
    { name: 'dev-a.apk', mtimeMs: 300 },
    { name: 'dev-b.apk', mtimeMs: 200 },
    { name: 'uat-a.apk', mtimeMs: 100 },
  ];
